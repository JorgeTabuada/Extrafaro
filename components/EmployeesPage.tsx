...
import { Users, Plus, Edit3, Trash2, User, AlertCircle, Search, Save, X, MapPin, Calendar, Euro, Cloud, CloudOff, RefreshCw, Phone, Mail, Filter } from "lucide-react"
          ...
          {filteredEmployees.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredEmployees.map((employee) => {
                ...
                return (
                  <Card key={employee.id} className="overflow-hidden hover:shadow-lg transition-all duration-300 border-2 hover:border-blue-300">
                    <CardContent className="p-0">
                      {/* Header do card com foto */}
                      <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 text-center">
                        <div className="w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden border-4 border-white shadow-lg bg-white flex items-center justify-center">
                          {employee.photo ? (
                            <Image
                              src={employee.photo}
                              alt={employee.name}
                        ...
                        <Badge className={`${employeeType.color} text-white px-3 py-1`}>
                          {employeeType.label}
                        </Badge>
                      </div>
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <div className="flex justify-center">
                          <Badge className={employeeState.color + " px-4 py-1"}>
                            {employeeState.label}
                          </Badge>
                        </div>
                        ...
                        {/* Disponibilidade - versão simplificada */}
                        <div className="text-center text-sm text-gray-600">
                          {WEEKDAYS.filter((day) => employee.weekAvailability[day.key]).length > 0 ? (
                            <span>
                              Disponível {WEEKDAYS.filter((day) => employee.weekAvailability[day.key]).length} dia(s)/semana
                            </span>
                          ) : (
                            <span className="text-gray-400">Sem disponibilidade definida</span>
                          )}
                        </div>
