...
export const syncEmployeesToSupabase = async (
  ...
  try {
    ...
    const employeesForDB = employees.map(emp => ({
      id: emp.id,
      name: emp.name,
      type: emp.type,
      state: emp.state,
      ...
      week_availability: emp.weekAvailability || {},
      hour_availability: emp.hourAvailability || {},
      custom_hourly_rate: emp.customHourlyRate || null,
      photo_url: emp.photo || null,
      citizen_card_number: emp.citizenCardNumber || null,
      citizen_card_file_url: emp.citizenCardFile || null,
      driving_license_number: emp.drivingLicenseNumber || null,
      driving_license_expiry: emp.drivingLicenseExpiry || null,
      driving_license_file_url: emp.drivingLicenseFile || null,
      contract_file_url: emp.contractFile || null,
    }))

    const { data, error } = await supabase
      .from('employees')
      .upsert(employeesForDB, { onConflict: 'id' })
...
export const loadEmployeesFromSupabase = async (
  ...
  try {
    ...
    const employeesForApp = (data || []).map((emp: any) => ({
      id: emp.id,
      name: emp.name,
      type: emp.type,
      state: emp.state,
    ...
    return { success: true, data: employeesForApp, message: `${data?.length || 0} colaboradores carregados!` }
  } catch (error) {
    console.error('Erro ao carregar colaboradores:', error)
    return { success: false, message: 'Erro interno ao carregar' }
  }
